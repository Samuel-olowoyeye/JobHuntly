...
const SmallCards = () => {
  return (
    <div className='gap-4 lg:gap-8 grid grid-cols-1 lg:grid-cols-4 px-4 lg:px-0 lg:py-[48px] pt-6 w-full h-auto'>
      {category.map((category) => (
        <div key={category.label} className='group lg:z-10 flex flex-row lg:flex-col hover:bg-[#4640DE] border border-[#D6DDEB] lg:w-[274px] xl:w-[320px] h-[84px] lg:h-[214px] group-hover:text-[#ffffff]'>
          <div className='group lg:mx-8 my-[22px] lg:my-8 mr-8 ml-4 text-[#4640DE] group-hover:text-[#ffffff]'>
            {category.icon} 
          </div>
        
        
        <div className='flex flex-col my-4 lg:my-0 lg:mr-0 px-4 lg:px-0 w-full'>
          <h2 className='group lg:px-8 lg:pb-3 font-Red_Hat_Display font-extrabold text-[#25324B] text-[20px] group-hover:text-[#ffffff] lg:text-2xl'> {category.label} </h2>
          
          
          <div className='group flex flex-row justify-between lg:justify-start lg:gap-4 lg:px-8 font-Epilogue text-[#7C8493] text-[16px] hover:text-[#ffff] group-hover:text-[#ffffff] lg:text-lg'>
            <h3>{category.total} jobs available </h3>
            <ArrowRight className='group lg:ml-0 w-[24px] h-[24px] text-[#25324B] group-hover:text-[#ffffff]' />
            </div>
